...
export const ProductUpdate = () => {
    ...
    useEffect(
            ...
            clientHttp({
                ...
                let productResponse = res.data;
                productResponse.expiration = productResponse.expiration.slice(0,10);
        ...
        <div>
            ...
            <Form noValidate validated={validated} onSubmit={(evt) => handleSubmit(evt)}>
                ...
                <Form.Group className="mb-3" controlId="formProductNotes">
                    ...
                    <Form.Select 
                        onChange={(evt) => handleChange(evt.target.value, "productTypeId")} 
                        required
                        value={
                            productTypes.filter(type => type.id === product.productTypeId).length > 0 ?
                            productTypes.filter(type => type.id === product.productTypeId)[0].id :
                            ""
                        }
                    >
                        <option value="">Seleccione un tipo de prodcuto</option>
                        {
                            ...
                            //         return (<option selected key={type.id} value={type.id}>{type.name}</option>);
                            //     }else {
                ...
                <Form.Group className="mb-3" controlId="formProductNotes">
                    ...
                    <Form.Select 
                            onChange={(evt) => handleChange(evt.target.value, "brandId")} 
                            required 
                            value={
                                brands.filter(brand => brand.id === product.brandId).length > 0 ?
                                brands.filter(brand => brand.id === product.brandId)[0].id :
                                ""
                            }
                        >
                        <option value="">Seleccione una marca</option>
                        {
                            ...
                            //         return (<option selected key={brand.id} value={brand.id}>{brand.name}</option>);
                            //     }else {
